...
from vendor import bottle, beanstalkc
from vendor.bottle import route, hook, request, abort, error, \
	post, delete, get, put, default_app, static_file
from vendor.bottle.ext import sqlalchemy as sabottle
...
def handle_500_error(http_error):
	logging.exception(http_error.exception)
	logging.error(http_error.traceback)
	logging.error(http_error.body)
	return json.dumps({'status': 1, 'error': "The server encountered an internal error (see server logs for details)"})
...
def check_auth():
	...
	if 'X-Auth-Token' in request.headers and \
		...
		if auth_token != expected_token:
			logging.error("Invalid Token:[%s]" % (auth_token,))
	...
	with database.session_scope() as session:
		...
		if user is None:
			logging.info("Unauthorized access attempt with token: {}".format(token))
			abort(403)
		if user.token_issued_at + datetime.timedelta(minutes=30) < datetime.datetime.utcnow():
			logging.info("Token expired: {}".format(token))
			abort(403)
...
def notify(provider, db):
	logging.info("Received notification from provider '{}': '{}' ".format(provider, request.json))
	...
	if 'full_name' not in repo:
		...
		if matches:
			logging.info("notify match:[%s]" % str(json.dumps(matches.groups())))
...
def environments_servers(environment_id, db):
	...
	releases = execution.concurrent_get_release_status(
		zip(hosts, repeat(env.target_folder()))
	)
...
@route('/api/status')
def status():
	return json.dumps({'status': 0, 'message': 'Deployer API is up and running'})
...
@get('/api/repositories/byname/<repo_name>')
@requires_logged
...
def environments_start_deployment(environment_id, db):
	...
	if 'X-Impersonate-Username' in request.headers:
		...
		logging.info("User {} impersonated user {} in order to deploy in the environment {}".format(request.account.username, impersonated.username, environment_id))
...
@get('/favicon.ico')
def favicon(db):
	return static_file('favicon.ico', WEB_ROOT)
...
# firmware is a legacy route, and will be removed
@get('<path:re:^(?!.*firmware).*>')
...
class ApiWorker(object):
	...
	def __init__(self, config_path, config, notifier, websocket_notifier, authentificator):
		app = bottle.app()
