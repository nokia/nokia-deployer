...
class Environment(Base):
	...
	remote_repo_path = sa.Column(sa.String(255), nullable=False, default='')
	production_folder = sa.Column(sa.String(255), nullable=False, default='')
	...
	def local_repo_directory_name(self):
		# Allow '/' for retrocompatibility and not having to clone again large repos (withings-sas)
	...
	def release_path(self, branch, commit):
		...
		In other words, this is where the code to de deployed is copied on the remote server.
		"""
	...
	def target_folder(self):
		"""
		Return the path where the project should be ultimately deployed.
		If the deployment method is 'inplace', it's the same as the release_path.
		"""
		return os.path.join(self.remote_repo_path, self.production_folder)
