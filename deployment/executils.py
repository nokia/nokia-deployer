	...
	try:
		...
		while p.poll() is None:
			...
			if (now - start) > datetime.timedelta(seconds=timeout):
				...
				logging.error("cmd:[%s] timeout! so far: stdout:[%s] stderr:[%s]" % (cmd, stdout, stderr))
		...
		stdout = "".join(outputs[0])
		stderr = "".join(outputs[1])
		logging.debug("cmd:[%s] stdout:[%s] stderr:[%s]" % (cmd, stdout, stderr))
		return (p.returncode, stdout, stderr)
	except Exception as e:
		logging.error(traceback.format_exc(e))
		logging.error("error:[%s] cmd:[%s]" % (str(e), cmd))
		return (1, "", str(e))
