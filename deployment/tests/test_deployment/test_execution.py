...
class TestSteps(unittest.TestCase):
	...
	def _load_fixtures(self, session):
		...
		entities = [
			m.Environment(id=1, repository_name="another repo", name="dev", clusters=[clusters[1]]),
			m.Environment(id=2, repository_name="my repo", name="dev", clusters=[clusters[0]]),
	...
	def test_run_predeploy(self):
		try:
			...
			with open(os.path.join(repo_path, 'predeploy.sh'), 'w') as f:
				...
				f.flush()
			_, entries = self._unwind(execution.run_predeploy(repo_path, "dev", "abcde"))
			self.assertEqual(1, len(entries))
			entry = entries[0]
	...
	def test_run_deploy(self, mock_func):
		mock_func.side_effect = lambda *args, **kwargs: (0, "stdout", "stderr")
		...
		host = executils.Host("some-server", "scaleweb", 22)
		self._unwind(execution.run_deploy(host, '/home/scaleweb/project', 'dev', 'abcde'), assert_no_error=True)
