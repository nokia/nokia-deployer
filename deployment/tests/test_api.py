...
from deployment.vendor.bottle import request, tob, HTTPError, default_app
...
class TestApi(unittest.TestCase):
	...
	def _load_fixtures(self, session):
		...
		entities = [
			...
			m.Environment(id=1, repository_name="another repo", name="dev", remote_user="deploy"),
			m.Environment(id=2, repository_name="my repo", name="dev", deploy_branch="master", env_order=1, clusters=[clusters[0]]),
			m.Environment(id=3, repository_name="my repo", name="beta", deploy_branch="prod", env_order=2),
			m.Environment(id=4, repository_name="my repo", name="prod", deploy_branch="prod", env_order=3),
	...
	def test_insert_environment(self):
		self._set_body_json({
			...
			'remote_repo_path': '/remote/repo/path',
			'production_folder': 'project_folder',
			'auto_deploy': True,
