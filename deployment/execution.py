...
# TODO: use a LoggerAdapter instead (from the Python standard library)
class LogProxy(object):
	"""Prepend a string to messages that are logged (using the logging module)."""
	...
	TO_INTERCEPT = ['info', 'error', 'debug', 'warn']

	def __getattr__(self, name):
		if name not in self.TO_INTERCEPT:
...
class Deployment(object):
	...
	def __init__(self, deploy_id, general_config, notifier, artifact_detector):
		...
		self.log = LogProxy("[deploy {}]".format(deploy_id))
	...
	def _get_artifact(self, local_repo_path, repo, mail_sender, mail_test_report_to):
		...
		if self.artifact.should_run_predeploy_scripts():
			# Predeploy script
			run_step(self, run_predeploy, local_repo_path, environment.name, self.view.commit)
	...
	def _copy_to_remotes(self, cluster, mail_sender, mail_test_report_to):
		...
		for host, server in zip(hosts, cluster.activated_servers):
			...
			run_step(self, run_deploy, host, os.path.join(environment.remote_repo_path, environment.production_folder), environment.name, self.view.commit)
	...
	def execute(self):
		...
		with database.session_scope() as session:
			try:
				...
				self.log.error('Unhandled error during deployment: {}'.format(traceback.format_exc(e)))
...
def run_predeploy(working_directory, environment_name, commit):
...
def run_deploy(host, remote_working_directory, environment_name, commit):
...
# Helpers
#########
def _get_git_release(host, target_folder, timeout=4):
	remote_path = os.path.join(target_folder, ".git_release")
	cmd = ['cat', remote_path]
...
def get_release_status(host, target_folder, timeout=4):
	(status, stdout, stderr) = _get_git_release(host, target_folder, timeout)
	if status != 0:
...
def concurrent_get_release_status(targets, timeout=4):
	"""
	Args:
		target (list of tuples): a list of (host, target_folder)
	"""
	if len(targets) == 0:
		return []
...
def run_test(environment, branch, commit, host, mail_sender, local, local_repo_path=None, mail_report_to=None):
	...
	if local:
		...
		remote_working_directory = os.path.join(environment.remote_repo_path, environment.production_folder)
		tests_defined = remote_check_file_exists(os.path.join(remote_working_directory, "tests/run_tests.sh"), host)
		cmd = (exec_script_remote, host, remote_working_directory, "tests/run_tests.sh", [environment.name, host.name, branch, commit])
...
def haproxy_action(haproxy_host, haproxy_keys, haproxy_auth, expected_status, changeto_status):
	...
	for ha_backend, ha_server in normalized_haproxy_keys:
		...
		logging.info("HAProxy current status of [%s/%s]: [%s] expected:[%s]" % (ha_backend, ha_server, ha_status, expected_status))
	...
	for ha_backend, ha_server in normalized_haproxy_keys:
		...
		if changeto_status == HAProxyAction.DISABLE and ("UP" in ha_status):
			logging.info("HAProxy change status of [%s/%s] from [%s] to [%s]" % (ha_backend, ha_server, ha_status, changeto_status))
			...
			ha_ret = "OK"
			logging.info("HAProxy status already OK for [%s/%s] [%s] == [%s]" % (ha_backend, ha_server, ha_status, changeto_status))
